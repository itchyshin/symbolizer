...
equation for one [`lm()`](https://rdrr.io/r/stats/lm.html) or `lmer()`
model. Reach for `symbolizer` when you need to understand the model —
across multi-submodel distributional fits, mixed models,
latent-variable, spatial, meta-analytic, or additive structures — its
assumptions, what each coefficient means on a natural scale, both
notations side by side, and how your data actually flows through the
matrices.
...
> builds the same kind of model in four rungs —
> [`lm()`](https://rdrr.io/r/stats/lm.html) →
> [`lm()`](https://rdrr.io/r/stats/lm.html) with a factor → `lmer()`
> with random intercepts → `drmTMB` location-scale — using one shared
> dataset and showing what new line each rung adds to the symbolic
> equation. That’s the recommended on-ramp.
...
([`symbolizer_capabilities()`](https://itchyshin.github.io/symbolizer/reference/symbolizer_capabilities.md))
is the source of truth — any class / family / component not marked
Stable or First slice there will be refused by
[`symbolize()`](https://itchyshin.github.io/symbolizer/reference/symbolize.md).
