...
`summary(fit)$p.coeff` / `summary(fit)$se`. Smooth-term inference is
summarised separately in `sym$metadata$smooths` (basis dimension, edf,
p-value per smooth) rather than a single coefficient row.
