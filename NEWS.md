...
* New: `compare_symbolic(sym_a, sym_b)` returns a structural diff
  ...
  `diff_assumptions` (status on each side plus a `same_status` flag).
  S3 class `symbolic_comparison`; `print()` produces a structured cli
  block and `knit_print()` produces side-by-side markdown tables.
