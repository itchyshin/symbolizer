...
<figure>
<img src="man/figures/three-views-widget.png"
alt="Three-views widget for a drmTMB location-scale fit" />
<figcaption aria-hidden="true">Three-views widget for a drmTMB
location-scale fit</figcaption>
</figure>
...
[`vignette("symbolizer-ladder")`](articles/symbolizer-ladder.html),
which is also the recommended on-ramp — it builds a model from `lm()` to
location-scale one rung at a time and uses the same `symbolize()` call
at every step.
