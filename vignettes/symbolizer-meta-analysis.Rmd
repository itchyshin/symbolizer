...
*Known limitation (v0.22.1):* the widget's per-tab equations currently
surface only the iid random-effect tier (`study_ID`); the phylogenetic
tier `phylo(1 | phylogeny, tree = ...)` is detected in
`sym$metadata$phylo_representation` but its variance row is not yet
propagated to the `$random_effects` table that drives Tabs 1–3. The
**math** in §4.1 above and the `metafor` Face below are the
authoritative views of the full two-tier random-effect structure.
Fixing this is tracked as a v0.22.1.1 follow-up.
