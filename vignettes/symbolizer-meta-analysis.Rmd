...
When studies are on related species, the between-study variance
splits into a phylogenetic piece (constrained by $\mathbf{A}$) and a
non-phylogenetic piece. The deep Face is
`metafor::rma.mv(yi, V = vi, random = ~ 1 | species,
                 R = list(species = A))`.

Cross-link: the v0.21.4-redo article
[`symbolizer-structural-dependence.html`](symbolizer-structural-dependence.html)
covers the **structural dependence** view of the same model (without
the sampling-tier $v_k$). This article's §4 will combine the two: known
