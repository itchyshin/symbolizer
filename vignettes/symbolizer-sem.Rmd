...
`node` column, so you can work with the whole system in R. The rendered
widget below shows the same node equations without exposing raw LaTeX in
the article:

```{r psem-equations, results = "hide"}
equations(sym)
```

`as_latex()` concatenates the per-node LaTeX under `## Node:` headers,
ready to splice into a manuscript. The command is run during the vignette
build, but the raw LaTeX string is not printed in the HTML article:

```{r psem-latex, results = "hide"}
cat(as_latex(sym))
```
