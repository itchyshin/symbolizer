...
`log()`-transformed predictor takes the `transformation` role and
currently has no template, so its row is silently dropped. The
coefficient is still in `sym$fixed_effects` and rendered in the
equations; it just does not yet get an English reading.
