...
`stats::lm` / `stats::glm`, `metafor`, `mgcv`, and `phylolm`.
...
`MCMCglmm`, `glmmTMB`, and `gllvmTMB`, plus the standalone `phylolm` PGLS
extractor — see `vignette("symbolizer-structural-dependence")`.
...
| `notation_bridge()` index ↔ matrix notation cross-reference | First slice |
...
`symbolize()`, `explain()`, `as_latex()`, `equations()`,
`symbol_table()`, `assumption_table()`, `formula_bridge()`,
`notation_bridge()`, `parameter_interpretation()`,
`variance_partition()`, `icc()`, `model_card()`, `as_dag()`,
`compare_symbolic()`, `methods_text()`, `warning_table()`,
`group_means()`, `group_slopes()`, `expand()`,
`as_html_three_views()`, `as_pdf_three_views()`,
`simulate_recipe()`, `symbolizer_capabilities()`.
