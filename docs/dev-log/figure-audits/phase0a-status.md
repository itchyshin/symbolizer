...
- 2026-05-26 ~02:xx — V1 Florence-lens on roadmap.html DONE in ~6 min. 5 new defects B92-B96. Layout/rendering clean (no overflow, no slug artifacts, no LaTeX leaks). B92 is a Pattern A escape leak (pipe-escape in inline code). B93-B96 are all Pattern F (claim/code drift) manifestations — the roadmap is ~6 versions stale and has 4 distinct factual mismatches with current git state. Plan catalog now B1-B96 + RF1-RF4; patterns A-FF (32 patterns still — no new pattern needed; B92-B96 all fold under existing A + F).
- Dispatching V1 on index.html next (light audit — article ordering).
- 2026-05-26 ~02:xx — V1 Florence-lens on index.html DONE in ~4 min. 5 new defects B97-B101 + 2 new patterns GG (pkgdown build doesn't realize _pkgdown.yml) and HH (hero/logo CSS overlap). B9 root cause is bigger than the maintainer thought: ladder and meta vignettes have .Rmd sources but were NEVER BUILT into docs/articles/ — that's why "Building up" precedes get-started (it's the only thing left after ladder went missing). Plan catalog now B1-B101 + RF1-RF4; patterns A-HH (34 patterns).
