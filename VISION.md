...
surface (HTML widget, vignette, README plot, pkgdown page, PDF export)
must pass a Florence visual check before the surrounding work is
declared done. Florence leads, but every role contributes: Pat sees
reader flow, Fisher sees uncertainty honesty, Rose sees stale claims,
Darwin sees biological coherence (logged traits for comparative
analyses; across-species ≠ within-species), Grace sees render evidence,
Boole sees syntax, Noether sees math correctness. The
`widget-visual-audit` checklist in `CLAUDE.md` is the mandatory
operationalisation.
...
When in doubt, stop. Re-read. The maintainer’s time spent saying “this
is broken” costs more than my time spent looking before I claim done.
